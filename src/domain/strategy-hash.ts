import type { StrategyResearchReceipt } from "./strategy-schema"
...
const scheduleWord = (schedule: Uint32Array, index: number): number => schedule[index] ?? 0
...
export const sha256Hex = (value: string): string => {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 16; index < 64; index += 1) {
      const previous15 = scheduleWord(schedule, index - 15)
      const sigma0 = rotateRight(previous15, 7) ^ rotateRight(previous15, 18) ^ (previous15 >>> 3)
      const previous2 = scheduleWord(schedule, index - 2)
      const sigma1 = rotateRight(previous2, 17) ^ rotateRight(previous2, 19) ^ (previous2 >>> 10)
      schedule[index] =
        (scheduleWord(schedule, index - 16) +
          sigma0 +
          scheduleWord(schedule, index - 7) +
          sigma1) >>>
        0
    }
...
export const deriveReceiptTemplateHash = (receipt: StrategyResearchReceipt): string => {
